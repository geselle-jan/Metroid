...
function preload() {
    ...
    game.load.tilemap('srx1', 'assets/rooms/srx1.json', null, Phaser.Tilemap.TILED_JSON);
    game.load.image('collision', 'assets/tilesets/collision.png');
    game.load.image('yellow', 'assets/tilesets/yellow.png');
    game.load.image('srx', 'assets/tilesets/srx.png');
...
function update() {
    if (samus.sprite.position.x >= g.map.widthInPixels - 34) {
        test();
    }
...
function test () {
    ...
    deco1.destroy();
    deco2.destroy();
    g.collisionLayer.destroy();



    g.map = game.add.tilemap('srx1');

    bg = game.add.tileSprite(0, 0, g.map.widthInPixels, g.map.heightInPixels, g.map.images[0].properties.key);
    ...
    samus.sprite.position.setTo(g.map.objects.doors[0].x + 32, g.map.objects.doors[0].y + 16);
    samus.sprite.bringToTop();
